// Supabase Edge Function: Automated Job Application using Puppeteer
// This function fully automates job applications server-side using Puppeteer (more compatible with Deno)
...
// Fill form fields using Puppeteer
    ...
    try {
      ...
      if (clearFirst) {
        await page.evaluate((sel) => {
          const elem = document.querySelector(sel) as HTMLInputElement | HTMLTextAreaElement
          if (elem) elem.value = ''
        }, selector)
      }
...
async function uploadResume(page: any, data: ApplicationData): Promise<boolean> {
  try {
    ...
    // Upload file using Puppeteer
    const fileInputElement = await page.$(fileSelectors.find(s => fileInput) || 'input[type="file"]')
    if (fileInputElement) {
      await fileInputElement.uploadFile(resumePath)
...
async function automateApplication(request: AutoApplyRequest): Promise<{
  ...
  try {
    ...
    await page.goto(request.jobUrl, { waitUntil: 'networkidle2', timeout: 30000 })
